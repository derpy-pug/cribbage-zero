...
public:
  ParseCommandLineArgs(int argc, char **argv) : argc(argc), argv(argv) {
    int ret = parse();
    if (ret != 0) {
   ...
   */
  std::pair<Hand, bool> string_to_hand(std::string hand_str) const {
  ...
  std::pair<Card, Card> string_to_discards(std::string discard_str) const {
  ...
  std::pair<Statistic, ScoreType>
  string_to_sort_by(std::string sort_by_str) const {
