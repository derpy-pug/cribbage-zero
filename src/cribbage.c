...
void discard_turn(Hand *hand, Hand *crib, PlayerInfo *player, char is_my_crib,
  ...
  while (tries--) {
    ...
    if (discards[0] < hand->length && discards[1] < hand->length) {
      ...
      if (player->type == HUMAN)
        printf("Invalid discards: Duplicate card\n");
      continue;
    }
    if (player->type == HUMAN)
      printf("Invalid discards: Index out of bounds\n");
  }
  if (tries <= 0) {
...
typedef enum { NO_GO, GO, GO_TWICE } GoState;
...
Winner play_phase(const Hand hand1, const Hand hand2, PlayerInfo *dealer,
                  PlayerInfo *pone, Card cut, Board *board) {
  ...
  Hand dealer_hand = init_hand(_hand1_played, 4);
  Hand pone_hand = init_hand(_hand2_played, 4);

  // Keep all played cards for history
  // Card _all_played_cards[8];

  Card _pile[8];
  CardPile pile = init_card_pile(_pile);
  ...
  while (1) {
    ...
    if (winner != NO_WINNER) {
      if (turn == 0)
        ...
        return winner == DEALER ? PONE : DEALER;
    }
    turn = !turn;
    round++;
  }
